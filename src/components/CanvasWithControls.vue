<template>
    <div class="canvas-with-controls">
      <div class="controls">
        <button @click="toggleOtherObjects">Hide other: {{ !hideOtherObjects }}</button>
        <MagnificationControls @input="handleMagnificationChange"></MagnificationControls>
      </div>
      <div class="canvas">
        <Canvas :ship="ship" :draw-other-objects="hideOtherObjects" :other-objects="otherObjects" :magnification="magnification" :background="background" :canvas-size="canvasSize"></Canvas>
      </div>
    </div>
  </template>
  ...
  // Define props for ship, magnification, background, and canvas size
  defineProps({
    ship: Object as () => Ship,
    background: Boolean,
    canvasSize: Object,
